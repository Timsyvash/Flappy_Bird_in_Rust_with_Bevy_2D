...
const FLAP_FORCE : f32 = 200.;
const GRAVITY : f32 = 350.;
    ...
    if let Ok((mut bird, mut transform)) = bird_query.get_single_mut() {
        ...
        if transform.translation.y <= -game_manager.window_dimensions.y / 2. {
            ...
            for (pipe_transform, _entity) in obstacle_query.iter() {
                if (pipe_transform.translation.y - transform.translation.y).abs() < OBSTACLE_HEIGHT * PIXEL_RATIO / 2.
                && (pipe_transform.translation.x - transform.translation.x).abs() < OBSTACLE_WIDTH * PIXEL_RATIO / 2. {
                    dead = true;
                    break;
                }
            }
